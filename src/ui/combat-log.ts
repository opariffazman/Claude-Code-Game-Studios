 ...
 * Green = damage, Red = destroyed, Yellow = wallpaper hit, Gold = milestone.
 */
import { Container, Text, TextStyle, Graphics } from 'pixi.js';

const MAX_ENTRIES = 8;
const COLORS = {
  damage: 0x1a6b1a,    // dark green — readable on light parchment
  destroyed: 0x8b1a1a, // dark red — readable on light parchment
  wallpaper: 0x8b7a1a, // dark gold — readable on light parchment
  milestone: 0xcc8800, // amber — milestone achievements stand out on parchment
};
...
export class CombatLog {
  ...
  /** Add a new log entry */
  addEntry(toolName: string, targetName: string, type: 'damage' | 'destroyed' | 'wallpaper' | 'milestone'): void {
    const color = COLORS[type];
    const prefix = type === 'destroyed' ? 'DESTROYED'
      : type === 'wallpaper' ? 'CRACKED'
      : type === 'milestone' ? ''
      : 'HIT';
    const text = prefix ? `${toolName} ${prefix} ${targetName}` : `${toolName} ${targetName}`;
  ...
  private _rebuild(latestColor: number): void {
    ...
    for (let i = 0; i < this._entryData.length; i++) {
      ...
      const style = new TextStyle({
        fontSize: 11,
        fill: isLatest ? latestColor : 0x555555,
